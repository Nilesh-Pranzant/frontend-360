...
const AddProperty = ({ onClose, onSuccess }) => {
  ...
  const validateForm = () => {
    ...
    if (!form.property_name.trim()) {
      ...
      newErrors.property_name = "Building name must be at least 3 characters";
      isValid = false;
    } else if (form.property_name.length > 150) {
      newErrors.property_name = "Building name cannot exceed 150 characters";
      isValid = false;
    }
  ...
  const validateField = (name, value) => {
    switch (name) {
      ...
      case "property_name":
        ...
        if (value.length < 3) return "Building name must be at least 3 characters";
        if (value.length > 150) return "Building name cannot exceed 150 characters";
        return "";
